...
int clear_garbage(char *inbuf, size_t n)
{
	...
	for (i = 0; n > 0 && i < n;)
	{
		if (isbase64(inbuf[i]) || inbuf[i] == '=')
		{
			i++;
		}
		else
		{
			memmove(inbuf + i, inbuf + i + 1, --n - i);
		}
	}
	inbuf[n] = '\0';
	return 0;
...
static inline char *get_4(struct base64_decode_context *ctx,
		  ...
		  const char *in, size_t inlen, char *out, size_t * outlen)
{
	...
	while (true)
	{
...

		{
			...
			if (inlen == 0
			    || (inlen < 4 && !flush_ctx && ignore_newlines))
			{
				inlen = 0;
				break;
			}
...
int main_example(int argc, char **argv)
{
    ...
    char *in = argv[2];
    char *out = NULL;
    size_t b64_att_len = 0;

    if (*type == 'e') {
            b64_att_len = base64_encode_alloc(in, strlen(in), &out);
    } else {
